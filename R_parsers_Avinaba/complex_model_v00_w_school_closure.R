...
SIM_SCRIPT_NAME = 'complex_model_v00_w_school_closure_025_for_365d_1case'
...
LIST_OF_CONTACT_MATRIX_SANS_SCHOOL_FILEPATH  <- 
  list.files(CONTACT_MATRIX_DIR, full.names = TRUE, pattern="contact\\_matrix\\-without\\-schools\\-real\\.csv$", ignore.case = TRUE)
...
# TODO: infection probability w.r.t infectivity of the variant, vaccination status of potentially infected and contact time 
...
CONST_MAX_ACTIVE_CASES_TO_SHUTDOWN_SCHOOLS = 100
...
STATELIST_ACTIVE_CASES <- which(colnames(transition_matrix) == "00_latent_infections_isolated" | 
                                  colnames(transition_matrix) == "01_latent_infections_isolated" | 
                                  colnames(transition_matrix) == "02_latent_infections_isolated" | 
...
for(day_index in 2:TOTAL_SIMULATION_DAYS){
      ...
      if(COUNT_OF_POSSIBLE_INFECTIOUS_CONTACTS > 0) {
        ...
        for(contact_index in list_of_infectious_contacts){
          ...
          if(PREV_STATE_NAME == "received_dose1"){
            ...
            vaccine_odds <- scaleVaccineOddsLinear(dose1_received_on) * vaccine_odds
          }
            ...
            dose1_received_on <- strtoi(infection_hist_mat[person_id, "dose 1 on"])
            vaccine_odds1 <- scaleVaccineOddsLinear(dose1_received_on) * vaccine_odds1
            ...
            vaccine_odds2 <- scaleVaccineOddsLinear(dose2_received_on) * vaccine_odds2
  ...
  if(COUNT_ACTIVE_CASES > CONST_MAX_ACTIVE_CASES_TO_SHUTDOWN_SCHOOLS){
    cat("\n >>> On day index: ", day_index, ", No. of active cases:",  COUNT_ACTIVE_CASES, ", School shutdown for sim day index: ", day_index + 1)
