...
SIM_SCRIPT_NAME = 'simulate_infection_00_batch_contact'
    ...
    if(NO_OF_CONTACT_MATRIX_BASED_TRANSITION > 0){
      ...
      if(TOTAL_PROBABILITY_OF_INFECTION > 0){
        ...
        if(runif(1) <= TOTAL_PROBABILITY_OF_INFECTION){
          ...
          ### INELEGANT SOLUTION ### 
          # Hardcoding selection of infection state on the basis of a "special" state == "CALCULATE_FROM_CONTACT_MATRIX"
          # 
          # Ideally should introduce another flag for each possible transition 
          # and set them TRUE for only those that require a mapping from contact matrix
          # 
