...
file_index = 1
total_flies = length(LIST_OF_SCHEDULE_JSON_FILEPATH)
...
for(JSON_FILENAME in LIST_OF_SCHEDULE_JSON_FILEPATH) {
  ...
  # Preprocess the final contacts based on interventions like School closures
  
  # Remove all records of schools 
  schedules_as_df <- filter(schedules_as_df, !location_id %in% which(distinct_locations$citisketch_location_type == "School"))
  ...
  OUTPUT_PAIRLIST_FILENAME_TOTAL <- paste(OUTPUT_FILENAME, "-contact_matrix-without-schools-real.csv", sep = "")
  ...
  cat("\nProcessed ", file_index, "/", total_flies)
