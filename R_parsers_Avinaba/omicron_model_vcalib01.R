...
#
# COUNT_RANDOM_SAMPLE_LIST <- 2^30 # = 1073741824, when divided by (5000 C 2) ~> 85 (should hold with pessimistic estimate for 5000 agents for 86 days)
...
SIM_SCRIPT_NAME = 'omicron_model_vcalib01_00375_for_30d_Feb09_22'

# Calibration mode: 
# TOTAL_SIMULATION_DAYS <- 10

# TOTAL_SIMULATION_DAYS <- 20
...
# ------ Start of analysis of coefficient of exponential growth ------
...
# To determine relative proportion of variants in target geography (Cambellton, NB)
{
  ...
  # voc_df <- voc_df %>% mutate_cond(`WHO label` == "Omicron" & `variant` == "B.1.1.529", 'Proportion' = 50)
  # voc_df <- voc_df %>% mutate_cond(`WHO label` == "Delta" & `variant` == "B.1.617.2", 'Proportion' = 50)
...
# Debug 
cat("\n")
cat("Sampled Y0: ", length(init_list_of_person_with_0_dose) / length(possible_vaccinated_agent_list), "\n")
cat("Sampled Y1: ", length(init_list_of_person_with_1_dose) / length(possible_vaccinated_agent_list), "\n")
cat("Sampled Y2: ", length(init_list_of_person_with_2_dose) / length(possible_vaccinated_agent_list), "\n")
cat("Sampled Y3: ", length(init_list_of_person_with_3_dose) / length(possible_vaccinated_agent_list), "\n")
...
for(day_index in 2:TOTAL_SIMULATION_DAYS){
  ...
  # Add omicron states 
  previous_day_infectious_df <- filter(previous_day_df, state_id %in% union(STATELIST_INFECTIOUS, STATELIST_INFECTIOUS_OMICRON))
      ...
      if(COUNT_OF_POSSIBLE_INFECTIOUS_CONTACTS > 0) {
        ...
        for(contact_index in list_of_infectious_contacts){
          ...
          # Note: This may fail for variants whose 3rd dose efficacy in-spite of the guard clauses of 2nd and 1st dose efficacies 
            ...
            vaccine_odds1 <- filter(active_voc_df, variant == infection_hist_mat[contact_index, "variant"])$'Vaccine dose 1 efficacy'
            vaccine_odds2 <- filter(active_voc_df, variant == infection_hist_mat[contact_index, "variant"])$'Vaccine dose 2 efficacy'
            vaccine_odds3 <- filter(active_voc_df, variant == infection_hist_mat[contact_index, "variant"])$'Vaccine dose 3 efficacy'
          ...
          if( random_coin_toss <= getInfection_probability_w_variant_w_vaccine(vaccine_odds, relative_infectivity, contact_time)){
            ...
            infection_WHO_label <- filter(active_voc_df, variant == infection_hist_mat[contact_index, "variant"])$'WHO label'
