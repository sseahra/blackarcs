...
CONST_CHOSEN_CONTACT_TRACING_STRATEGY <- "Conventional tracing"
...
# - Campbellton sims
# Debug:
# SIM_SCRIPT_NAME = 'LocalDebug_Campbellton_SIR_delay03_tracing_random_import_vcalib13_00300_1mat_for_210d_aggregated'
...
# Tag: @calib04
CONST_CHOSEN_CONTACT_MATRIX_TYPE <- "Location based"
...
if (CONST_CHOSEN_DISEASE_MODEL == "Complex") {
  ...
  # Infection parameters for people with 3rd dose of vaccination
  {
    ...
    # From Pre-symptomatic
    E(disease_model, path = c("03_pre_symptomatic_non_isolated", "03_mild_non_isolated") )$weight <- ( CONST_v_m + CONST_epsilon_serious_03 * (1 - CONST_v_m)  )* (1 - CONST_p) * CONST_KAPPA
    E(disease_model, path = c("03_pre_symptomatic_non_isolated", "03_mild_non_isolated") )$label  <- paste0("(3 → 5) = ", ( CONST_v_m + CONST_epsilon_serious_03 * (1 - CONST_v_m)  )* (1 - CONST_p) * CONST_KAPPA)

    E(disease_model, path = c("03_pre_symptomatic_non_isolated", "03_mild_isolated") )$weight <- ( CONST_v_m + CONST_epsilon_serious_03 * (1 - CONST_v_m)  ) * CONST_p * CONST_KAPPA
    E(disease_model, path = c("03_pre_symptomatic_non_isolated", "03_mild_isolated") )$label <- paste0("(3 → 6) = ", ( CONST_v_m + CONST_epsilon_serious_03 * (1 - CONST_v_m)  ) * CONST_p * CONST_KAPPA)

    E(disease_model, path = c("03_pre_symptomatic_non_isolated", "03_hospitalized") )$weight <- CONST_v_h * (1 - CONST_epsilon_serious_03) * CONST_KAPPA
    E(disease_model, path = c("03_pre_symptomatic_non_isolated", "03_hospitalized") )$label <- paste0("(3 → 7) = ", CONST_v_h * (1 - CONST_epsilon_serious_03)  * CONST_KAPPA)

    E(disease_model, path = c("03_pre_symptomatic_non_isolated", "03_hospitalized_ICU") )$weight <- CONST_v_c * (1 - CONST_epsilon_serious_03) * CONST_KAPPA
    E(disease_model, path = c("03_pre_symptomatic_non_isolated", "03_hospitalized_ICU") )$label <- paste0("(3 → 8) = ", CONST_v_c  * (1 - CONST_epsilon_serious_03) * CONST_KAPPA)



    E(disease_model, path = c("03_pre_symptomatic_isolated", "03_mild_isolated") )$weight <- ( CONST_v_m + CONST_epsilon_serious_03 * (1 - CONST_v_m)  ) * CONST_KAPPA
    E(disease_model, path = c("03_pre_symptomatic_isolated", "03_mild_isolated") )$label <- paste0("(4 → 6) = ", ( CONST_v_m + CONST_epsilon_serious_03 * (1 - CONST_v_m)  ) * CONST_KAPPA)

    E(disease_model, path = c("03_pre_symptomatic_isolated", "03_hospitalized") )$weight <- CONST_v_h * (1 - CONST_epsilon_serious_03) * CONST_KAPPA
    E(disease_model, path = c("03_pre_symptomatic_isolated", "03_hospitalized") )$label <- paste0("(4 → 7) = ", CONST_v_h  * (1 - CONST_epsilon_serious_03) * CONST_KAPPA)

    E(disease_model, path = c("03_pre_symptomatic_isolated", "03_hospitalized_ICU") )$weight <- CONST_v_c * (1 - CONST_epsilon_serious_03) * CONST_KAPPA
    E(disease_model, path = c("03_pre_symptomatic_isolated", "03_hospitalized_ICU") )$label <- paste0("(4 → 8) = ", CONST_v_c * (1 - CONST_epsilon_serious_03) * CONST_KAPPA)




    # From mild
    E(disease_model, path = c("03_mild_isolated", "03_recovered") )$weight <- CONST_ita
    E(disease_model, path = c("03_mild_isolated", "03_recovered") )$label <- paste0("(6 → r) = ", CONST_ita)

    E(disease_model, path = c("03_mild_non_isolated", "03_recovered") )$weight <- CONST_ita
    E(disease_model, path = c("03_mild_non_isolated", "03_recovered") )$label <- paste0("(5 → r) = ", CONST_ita)
...
asymptomatic_antigen_TNR <- available_covid_tests_df$`Symptomatic TNR`[ available_antigen_test_index ]
asymptomatic_antigen_FPR <- available_covid_tests_df$`Symptomatic FPR`[ available_antigen_test_index ]
...
# contact_matrix_index_lookup_list <- (NA, contact_matrix_index_lookup_list)
# Tag: @calib11
...
contactTrace <- function( traced_agent_id, tracing_day_index ){
  ...
  contact_matrix_index_list <- contact_matrix_index_lookup_list[tracing_window_days - 1]
  ...
  traced_close_contacts_df <- data.frame(agent_id = all_close_contact_indexes, contact_time = traced_list_of_contact_times[all_close_contact_indexes])
  traced_casual_contacts_df <- data.frame(agent_id = all_casual_contact_indexes, contact_time = traced_list_of_contact_times[all_casual_contact_indexes])
...
for(run_index in 1:TOTAL_SIMULATION_RUNS){
    ...
    # Transitions for both susceptibles and non-susceptibles
    {
        ...
        if ( (CONST_CHOSEN_DISEASE_MODEL == "SIR + Testing + Isolation") ||
          ...
          if(count_molecular_result_requested_yesterday_not_infected > 0){
            ...
            if(CONST_CHOSEN_DISEASE_MODEL ==  "SIR + Testing + Isolation"){
              ...
              if(count_molecular_result_requested_yesterday_removed > 0){
                ...
                # Change state
                STATE[output_state_molecular_result_susceptible, day_index - 1] <- output_state_molecular_result_removed
                
                
                # Aggregate agent ids receiving positive result
                removed_positive_result_ids <- molecular_result_requested_yesterday_susceptible_ids[output_state_molecular_result_removed %in% result_state_positive]
                
                # Aggregate agent ids receiving negative result
                removed_negative_result_ids <- molecular_result_requested_yesterday_susceptible_ids[output_state_molecular_result_removed %in% result_state_negative]
              }
          ...
          if( (CONST_CHOSEN_DISEASE_MODEL == "SIR + Testing + Isolation + Contact Tracing") ) {
            ...
            if(CONST_CONTACT_TRACING_DELAY > 0){
              ...
              # i.e. All neighbours of a particular agent who receives a positive
              # test result is tested same day. This process is not recursed further till the maximum tracing level is reached.
              ...
              for(tracing_level in 1:CONST_SAMEDAY_TRACING_LEVEL){
                ...
                # cat("\n  |- Already tested yesterday  : ", same_day_tested_yesterday_ids)
                # cat("\n  '- New traced contact to test: ", not_tested_yesterday_neighbours_agent_ids, "\n")
      ...
      # Check contacts of infectious agents to infect
        ...
        for(infector_id in previous_day_infectious_person_ids){
          ...
          if( GENERATE_EVENT_CONTACT_PAIRS && day_index >= EVENT_START_DATE && day_index <= EVENT_END_DATE){
            # With event contacts
            list_of_susceptibles <- intersect(list_day_x_person_x_contact_w_event[[contact_matrix_index_lookup_list[day_index - 1]]][[infector_id]], current_day_susceptible_person_ids)
          } else {
             # Without event contacts
            list_of_susceptibles <- intersect(list_day_x_person_x_contact[[contact_matrix_index_lookup_list[day_index - 1]]][[infector_id]], current_day_susceptible_person_ids)
          }
          ...
          for (susceptible_agent_id in list_of_susceptibles){
            ...
            if(CONST_CHOSEN_CONTACT_MATRIX_TYPE == "Aggregated"){
              ...
              # Load contact time including time spent in event with events during event duration
              if( GENERATE_EVENT_CONTACT_PAIRS && day_index >= EVENT_START_DATE && day_index <= EVENT_END_DATE){
                contact_time <- list_of_contact_matrix_w_event[[contact_matrix_index_lookup_list[day_index - 1]]][susceptible_agent_id, infector_id]
              } else {
                # Note, old code based on iterator
                contact_time <- contact_matrix[susceptible_agent_id, infector_id]
                # Maybe changed to the following
                # contact_time <- list_of_contact_matrix[[contact_matrix_index_lookup_list[day_index - 1]]][susceptible_agent_id, infector_id]
              }
              ...
              # Load contact time including time spent in event with events during event duration
              if( GENERATE_EVENT_CONTACT_PAIRS && day_index >= EVENT_START_DATE && day_index <= EVENT_END_DATE){
                person_specific_contact_indices <- which(list_day_x_person_x_contact_w_event[[contact_matrix_index_lookup_list[day_index - 1]]][[susceptible_agent_id]] == infector_id)

                # Get all person-specific contact times
                list_of_contact_times <- list_day_x_person_x_contact_time_w_event[[contact_matrix_index_lookup_list[day_index - 1]]][[susceptible_agent_id]]
                # Filter times for this particular contact
                list_of_contact_times <- list_of_contact_times[person_specific_contact_indices]

                # Get all person-specific contact location types
                list_of_contact_location_types <- list_day_x_person_x_contact_location_type_w_event[[contact_matrix_index_lookup_list[day_index - 1]]][[susceptible_agent_id]]
                # Filter location types for this particular contact
                list_of_contact_location_types <- list_of_contact_location_types[person_specific_contact_indices]

                # Get all person-specific contact location types
                list_of_contact_location_ids <- list_day_x_person_x_contact_location_id_w_event[[contact_matrix_index_lookup_list[day_index - 1]]][[susceptible_agent_id]]
                # Filter location types for this particular contact
                list_of_contact_location_ids <- list_of_contact_location_ids[person_specific_contact_indices]


              } else {
                person_specific_contact_indices <- which(list_day_x_person_x_contact[[contact_matrix_index_lookup_list[day_index - 1]]][[susceptible_agent_id]] == infector_id)

                # Get all person-specific contact times
                list_of_contact_times <- list_day_x_person_x_contact_time[[contact_matrix_index_lookup_list[day_index - 1]]][[susceptible_agent_id]]
                # Filter times for this particular contact
                list_of_contact_times <- list_of_contact_times[person_specific_contact_indices]

                # Get all person-specific contact location types
                list_of_contact_location_types <- list_day_x_person_x_contact_location_type[[contact_matrix_index_lookup_list[day_index - 1]]][[susceptible_agent_id]]
                # Filter location types for this particular contact
                list_of_contact_location_types <- list_of_contact_location_types[person_specific_contact_indices]

                # Get all person-specific contact location types
                list_of_contact_location_ids <- list_day_x_person_x_contact_location_id[[contact_matrix_index_lookup_list[day_index - 1]]][[susceptible_agent_id]]
                # Filter location types for this particular contact
                list_of_contact_location_ids <- list_of_contact_location_ids[person_specific_contact_indices]
              }
        ...
        for(person_index in 1:length(previous_day_non_susceptible_person_states)){
          ...
          if(PREV_STATE_INDEX != next_state_id){
            ...
            if(CONST_CHOSEN_DISEASE_MODEL == "Complex"){
              ...
              # Edge: "(unconfirmed) infected" ---Same day testing temp state "(isolated + unconfirmed) infected"--- Test Result (TPR) --> "(isolated + confirmed) infected"
              # Edge: "susceptible" -- Same day testing temp state "(isolated) susceptible" -- Test Result (FPR) --> (isolated + false positive) susceptible
              # Edge: "susceptible" -- Same day testing temp state "(isolated) removed" -- Test Result (FPR) --> (isolated + false positive) removed
  ...
  # if(day_index != TOTAL_SIMULATION_DAYS) # Hacky active debug data structures
  {
    cat("\n================================================\n")
