...
SIM_SCRIPT_NAME = 'simulate_infection_00'

# input files 

# contact matrix (pairlist)
CONTACT_MATRIX_AS_PAIRLIST_FILENAME = 'scenarioBenchmark/Schedule Benchmark A - Cambellton-contact_matrix-total-real.csv'
...
SUB_DIR = basename(CONTACT_MATRIX_AS_PAIRLIST_FILENAME)
...
#
# for simplicity/completeness assuming there's an unknown state transition probability for 
# latent infections(Exposed) to asymptomatic(Infected) as well
# There are early researches stating difference in inactivity for asymptomatic individuals
# Ref: 
...
# ** asymptomatic: infected and infectious and NOT showing symptoms (Infectious)
...
# Vaccinations
disease_model <- graph( edges = c("susciptible","received_dose1",  
                                  "received_dose1","received_dose2"),directed = TRUE)

# TODO: Vaccination/day parameters 
# E(disease_model, path = c("susciptible", "received_dose1") )$weight <- 0.1
...
# Infection states for 0 doses: 
  disease_model <- disease_model+  
                   graph( edges = c("latent_infections_isolated","asymptomatic",
                                    "latent_infections_isolated", "pre_symptomatic_isolated", 
                                    "latent_infections_isolated", "pre_symptomatic_non_isolated",
                                    
                                    "latent_infections_not_isolated","asymptomatic",
...
E(disease_model, path = c("latent_infections_isolated","asymptomatic") )$weight <- CONSTANT_NOT_VACCINATED
E(disease_model, path = c("latent_infections_isolated", "pre_symptomatic_isolated") )$weight <- CONSTANT_NOT_VACCINATED
E(disease_model, path = c("latent_infections_isolated", "pre_symptomatic_non_isolated") )$weight <- CONSTANT_NOT_VACCINATED

E(disease_model, path = c("latent_infections_not_isolated","asymptomatic") )$weight <- CONSTANT_NOT_VACCINATED
E(disease_model, path = c("latent_infections_not_isolated", "pre_symptomatic_isolated") )$weight <- CONSTANT_NOT_VACCINATED
E(disease_model, path = c("latent_infections_not_isolated", "pre_symptomatic_non_isolated") )$weight <- CONSTANT_NOT_VACCINATED

E(disease_model, path = c("pre_symptomatic_isolated", "mild_isolated") )$weight <- CONSTANT_NOT_VACCINATED
E(disease_model, path = c("pre_symptomatic_isolated", "mild_non_isolated") )$weight <- CONSTANT_NOT_VACCINATED
E(disease_model, path = c("pre_symptomatic_isolated", "hospitalized") )$weight <- CONSTANT_NOT_VACCINATED
E(disease_model, path = c("pre_symptomatic_isolated", "hospitalized_ICU") )$weight <- CONSTANT_NOT_VACCINATED

E(disease_model, path = c("pre_symptomatic_non_isolated", "mild_isolated") )$weight <- CONSTANT_NOT_VACCINATED
E(disease_model, path = c("pre_symptomatic_non_isolated", "mild_non_isolated") )$weight <- CONSTANT_NOT_VACCINATED
E(disease_model, path = c("pre_symptomatic_non_isolated", "hospitalized") )$weight <- CONSTANT_NOT_VACCINATED
E(disease_model, path = c("pre_symptomatic_non_isolated", "hospitalized_ICU") )$weight <- CONSTANT_NOT_VACCINATED

E(disease_model, path = c("asymptomatic", "recovered") )$weight <- CONSTANT_NOT_VACCINATED
E(disease_model, path = c("mild_isolated", "recovered") )$weight <- CONSTANT_NOT_VACCINATED
E(disease_model, path = c("mild_non_isolated", "recovered") )$weight <- CONSTANT_NOT_VACCINATED
E(disease_model, path = c("hospitalized", "recovered") )$weight <- CONSTANT_NOT_VACCINATED
E(disease_model, path = c("hospitalized", "recovered") )$weight <- CONSTANT_NOT_VACCINATED
E(disease_model, path = c("hospitalized_ICU", "recovered") )$weight <- CONSTANT_NOT_VACCINATED

E(disease_model, path = c("mild_isolated", "dead") )$weight <- CONSTANT_NOT_VACCINATED
E(disease_model, path = c("mild_non_isolated", "dead") )$weight <- CONSTANT_NOT_VACCINATED
E(disease_model, path = c("hospitalized", "dead") )$weight <- CONSTANT_NOT_VACCINATED
E(disease_model, path = c("hospitalized_ICU", "dead") )$weight <- CONSTANT_NOT_VACCINATED
...
# Attaach susciptible with latent_infections (Exposed) type states
disease_model <- disease_model + edge("susciptible", "latent_infections_isolated", 
                                      weight = "CALCULATE_FROM_CONTACT_MATRIX", 
                                      label = "CALCULATE_FROM_CONTACT_MATRIX")

disease_model <- disease_model + edge("susciptible", "latent_infections_not_isolated", 
                                       weight = "CALCULATE_FROM_CONTACT_MATRIX", 
                                       label = "CALCULATE_FROM_CONTACT_MATRIX")
...
STATE[ , 1] <- which( colnames(transition_matrix) == "susciptible" )
...
# Generate static infection probability
...
# Mapping to SEIR 
STATELIST_SUSCIPTIBLE <- which(colnames(transition_matrix) == "susciptible" | 
                                 colnames(transition_matrix) == "received_dose1" |
                                 colnames(transition_matrix) == "received_dose2")

STATELIST_EXPOSED <- which(colnames(transition_matrix) == "latent_infections_isolated" | 
                             colnames(transition_matrix) == "latent_infections_not_isolated")

STATELIST_INFECTIOUS <- which(colnames(transition_matrix) == "asymptomatic" | 
                                colnames(transition_matrix) == "pre_symptomatic_isolated" | 
                                colnames(transition_matrix) == "pre_symptomatic_non_isolated" | 
                                
                                colnames(transition_matrix) == "mild_isolated" | 
                                colnames(transition_matrix) == "hospitalized" | 
                                colnames(transition_matrix) == "hospitalized_ICU")
