...
# 2. Read daily new cases and remove non-outbreaks
...
LIST_OF_DAILY_ACTIVE_CASES_W_SCHOOL_CLOSURE_FILEPATH <-
  list.files(SCHOOL_CLOSURE_INPUT_DIR, full.names = TRUE, pattern="DAILY\\-NEW\\-CASES\\.csv$", ignore.case = TRUE)
...
LIST_OF_DAILY_ACTIVE_CASES_FILEPATH <-
  list.files(CONTROL_INPUT_DIR, full.names = TRUE, pattern="DAILY\\-NEW\\-CASES\\.csv$", ignore.case = TRUE)
...
ggplot() +
  ...
  xlab("Simulation day") + 
  # ylab("Active cases") +
  ylab("New cases (for runs with total infection > 2)") +
  ylim(0, 350) + 
